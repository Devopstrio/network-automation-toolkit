...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Automation Hub', path: '/', icon: LayoutDashboard },
    { name: 'Network Topology', path: '/topology', icon: Map },
    { name: 'Device Inventory', path: '/devices', icon: Server },
    { name: 'Config Management', path: '/configs', icon: FileText },
    { name: 'Policy Compliance', path: '/compliance', icon: ShieldCheck },
    { name: 'Automation Jobs', path: '/jobs', icon: Zap },
    { name: 'Drift Detection', path: '/drift', icon: Signal },
    { name: 'Rollback Console', path: '/rollback', icon: RotateCcw },
    { name: 'Security & ACLs', path: '/security', icon: Layers },
    { name: 'Cloud Networking', path: '/cloud', icon: Cloud },
    { name: 'Change Management', path: '/changes', icon: ArrowRightLeft },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-blue-400 bg-clip-text text-transparent">Net.Ops</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">NetDevOps Architect</p>
                <p className="text-xs text-slate-500 truncate">Infrastructure Automation</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search network devices, configurations, automation jobs..." 
